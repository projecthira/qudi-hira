...
class AwgPulsedODMRGui(GUIBase):
    ...
    # declare connectors
    odmrlogic1 = Connector(interface='RasterAwgPulsedODMRLogic')
    ...
    def on_activate(self):
        ...
        *.ui file and configures the event handling between the modules.
        ...
        self._mw.laser_readout_length_doubleSpinBox.setValue(self._odmr_logic.laser_readout_length)
        ...
        self.sigMwSweepParamsChanged.connect(self._odmr_logic.set_sweep_parameters,
                                             QtCore.Qt.QueuedConnection)
        self.sigPulseParamsChanged.connect(self._odmr_logic.set_pulse_parameters,
                                             QtCore.Qt.QueuedConnection)
    ...
    def on_deactivate(self):
        ...
        self.sigPulseParamsChanged.disconnect()
        ...
        self._mw.laser_readout_length_doubleSpinBox.editingFinished.disconnect()
    ...
    def update_status(self, mw_mode, is_running):
        ...
        if is_running:
            ...
            if mw_mode != 'cw':
                ...
                self._mw.laser_readout_length_doubleSpinBox.setEnabled(True)
    ...
    def update_parameter(self, param_dict):
        ...
        param = param_dict.get('laser_readout_length')
        if param is not None:
            self._mw.laser_readout_length_doubleSpinBox.blockSignals(True)
            self._mw.laser_readout_length_doubleSpinBox.setValue(param)
            self._mw.laser_readout_length_doubleSpinBox.blockSignals(False)

        param = param_dict.get('delay_length')
        if param is not None:
            self._mw.delay_length_doubleSpinBox.blockSignals(True)
            self._mw.delay_length_doubleSpinBox.setValue(param)
            self._mw.delay_length_doubleSpinBox.blockSignals(False)

        param = param_dict.get('pi_pulse_length')
        if param is not None:
            self._mw.pi_pulse_length_doubleSpinBox.blockSignals(True)
            self._mw.pi_pulse_length_doubleSpinBox.setValue(param)
            self._mw.pi_pulse_length_doubleSpinBox.blockSignals(False)
    ...
    def change_sweep_params(self):
        ...
        self.sigMwSweepParamsChanged.emit(start, stop, step, power, single_sweep_time)
        return
