...
class AWG:
    ...
    #     self.run_sequence_process = Process(target=self._run_sequence_from_list, args=(seq, self.hub, self.cards))
    #     print 'Starting upload process!'
    #     self.run_sequence_process.start()
    #     self.run_sequence_process.join()
    ...
    # def run_pulsed(self, taus, mwsequence, imwsequence="", dict_values=dict(), LaserOn=2000, LaserWait=2000, pre_laser_sequence='', pre_laser_sequence_i='', clocking=0):
    #
    #     #clocking => make the length of each measurement a multiple of clocking
    #
    #     self.uploading = True
    ...
    def set_loops(self, loops):
        for c in self.cards:
            c.set_loops(loops)
        # print('Loops set in the AWG to', loops)
...
class Card():
    ...
    def upload(self, number_of_samples, data=None, data1=None, marker0_data=None, marker1_data=None, marker2_data=None,
        ...
        # test[0:number_of_samples*2:2] = data + np.ma.masked_where(data<0, data, copy=False).mask*2**15 - marker0_data*2**15
        # test[1:number_of_samples*2:2] = data1 + np.ma.masked_where(data1<0, data1, copy=False).mask*2**14 - marker1_data*2**15 - marker2_data*2**14
        ...
        #     val -= 2**15
        #
        # pnBuffer[i*used_channels] = int(val)
        # test[i*used_channels] = int(val)
        #
        # if data1[i] < 0:
        #     val2 = int(data1[i] + 2**14)
        # else:
        #     val2 = int(data1[i])
        #
        # if marker2_data[i]:
