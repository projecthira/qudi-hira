...
class Lakeshore625SMPS(Base, MagnetInterface):
    """
    Main class for the Lakeshore 625 Superconducting Magnet Power Supply.
    ...
    def get_ids(self):
        return self.ask({'x': "*IDN?", 'y': "*IDN?", 'z': "*IDN?"})
    ...
    def heat_switch(self, axis, power=10, delay=5):
        """ This function enables heating of the PJSwitch,  which is a necessary
            step to conduct current to the coils.

            @param int power: Specifies the current needed to turn on the PSH 10mA to 125 mA
            @param int delay: Specifies the time needed to turn the PSH on or off: 5s to 100s
            @param string axis: desired axis (x, y, z)
            """
        power, delay = int(power), int(delay)
        if axis == "x":
            self.tell({'x': 'PSHS 1,{},{}'.format(power, delay)})
            self.tell({'x': 'PSH 1'})
        elif axis == "y":
            self.tell({'y': 'PSHS 1,{},{}'.format(power, delay)})
            self.tell({'y': 'PSH 1'})
        elif axis == "z":
            self.tell({'z': 'PSHS 1,{},{}'.format(power, delay)})
            self.tell({'z': 'PSH 1'})
        else:
    ...
    def initialize(self):
        ...
        answ_dict = self.ask({'x': "PSH?", 'y': "PSH?", 'z': "PSH?"})
    ...
    def target_field_setpoint(self, param_dict):
        """ TODO Function to set the target field (in T), which will be reached through the
    ...
    def ramp(self, param_list=None):
        ...
        if param_list is None:
            self.tell({'x': 'SETF {:.4G}'.format(self.field_dict['x'])})
            self.tell({'y': 'SETF {:.4G}'.format(self.field_dict['y'])})
            self.tell({'z': 'SETF {:.4G}'.format(self.field_dict['z'])})
        else:
            if 'x' in param_list:
                self.tell({'x': 'SETF {:.4G}'.format(self.field_dict['x'])})
    ...
    def ask_status(self, param_list=None):
        """ TODO: Function that returns the status of the coils ('x','y' and 'z') given in the
            param_dict
    ...
    def set_velocity(self, param_dict):
        """ TODO: Function to change the ramp rate  in T/s (ampere per second)
        ...
        if param_dict.get('x') is not None:
            ...
            if constraint_x > param_list[1]:
                tell_dict['x'] = 'CONF:RAMP:RATE:FIELD:' + str(param_list[0]) + ", " + str(param_list[1]) + ", " + str(
                    param_list[2])
            else:
        ...
        if param_dict.get('y') is not None:
            ...
            if constraint_y > param_list[1]:
                tell_dict['y'] = 'CONF:RAMP:RATE:FIELD:' + str(param_list[0]) + ", " + str(param_list[1]) + ", " + str(
                    param_list[2])
            else:
        ...
        if param_dict.get('z') is not None:
            ...
            if constraint_z > param_list[1]:
                tell_dict['z'] = 'CONF:RAMP:RATE:FIELD:' + str(param_list[0]) + ", " + str(param_list[1]) + ", " + str(
                    param_list[2])
            else:
    ...
    def get_velocity(self, param_list=None):
        """ TODO: Gets the current velocity for all connected axes.
    ...
    def set_magnetic_field_constant(self, field_constant):
        """Set the magnetic field constant of the magnet (in T/A)."""
        field_constant = float(field_constant)
        tell_dict = {'x': 'FLDS 0, {:.4f}'.format(field_constant),
                     'y': 'FLDS 0, {:.4f}'.format(field_constant),
                     'z': 'FLDS 0, {:.4f}'.format(field_constant)}
        self.tell(tell_dict)
        return 0

    def get_magnetic_field_constant(self):
        """ Get the magnetic field constant of the magnet (in T/A). """
        return self.ask({'x': 'FLDS?', 'y': 'FLDS?', 'z': 'FLDS?'})

    def set_quench_detection(self, rate_limit=1):
